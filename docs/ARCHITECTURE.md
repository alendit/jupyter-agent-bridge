...
- Cell-mutating and execution tools expose an opt-in `reveal_cell` toggle for cell-following behavior. It defaults to `false`, preventing automatic edit and execution actions from moving the notebook viewport. Explicit reveals preserve the active editor and cell selection. Range-based execution targets the notebook by URI, disables the editor command's own auto-reveal, and does not select its target cells. Explicit selection, richer viewport placement, and output focus stay on `reveal_notebook_cells` so the edit/execute surfaces keep a boolean-only presentation contract.
